...
The chambers produce the raw record and always have. The Clerk of the House and the Secretary of the Senate enter each roll call in the Journal. Since 1973 the House has taken most recorded votes by electronic machine, which multiplied how many it takes. But no government office publishes the whole record as one file. The dataset nearly everyone uses is Voteview, assembled by the political scientists Keith Poole, Howard Rosenthal, Jeffrey Lewis and their colleagues. They keyed the early Journals by hand and read the chambers' modern electronic feeds for the rest.
...
This chapter reads the roll-call file, `HSall_rollcalls.csv`: `r nn(TOTAL)` recorded votes, House and Senate, from the 1st Congress to the `r LASTC`th. The chambers also publish their own modern feeds: the Clerk's site for the House, the Senate's for itself. Voteview reconciles those into the same series that the hand-keyed Journals begin.
...
The same manufacture runs on it. Ideal-point scores for justices, of which Martin-Quinn scores are the best known, are built from patterns of agreement in exactly the way NOMINATE is built from roll calls. The [Supreme Court chapter](../scdb/scdb-brief.html) constructs one from nothing but agreement, so you can watch the machinery work on a court small enough to check by hand. And the [oral argument chapter](../oral-argument/oral-argument-brief.html) reads the Court's other record, the transcript, where the hard decision is what to count at all.
