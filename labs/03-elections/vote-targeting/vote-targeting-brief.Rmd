...
Four state releases, about 110 MB of precinct rows, became those `r n(nrow(nc))`. Every precinct in a county was added up, every contest but the governor's race was discarded, and every candidate was folded into `dem`, `rep` or `oth`. The four vote-method columns (election day, early, mail and provisional) were summed into one total, so the file cannot say *how* anyone voted, only how many did.

The cleaning had one problem in it that would have failed silently. The 2012 release is a quoted CSV with lowercase headers; the three later releases are tab-separated, with a different column set entirely. A script written for one layout does not crash on the other — it returns zero rows and hands back a tidy table missing an entire election. So the build reads each release on its own terms, treats a blank vote cell as zero, and refuses to finish unless it has all `r n(length(unique(nc$county)))` counties in all `r length(unique(nc$year))` years. It also re-derives the statewide totals against the certified canvass on every run, which is how the textbook's two stray figures came to be noticed at all.
...
**Figure 1. How concentrated each party's vote is.** Cumulative curves over ranked counties, because the question is how few places hold half of each party's vote, and a running total answers it directly. The Democratic curve reaches half the party's vote in **`r half_d` counties**; the Republican curve needs **`r half_r`**. The curves stay close over most of their length: concentration is largely a fact about where people live, not about which party they vote for.
...
- North Carolina State Board of Elections, precinct-level returns for the
  ...
  here. The statewide totals are re-checked against the published canvass
  every time they are assembled.
