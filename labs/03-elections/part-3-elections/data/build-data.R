# ---------------------------------------------------------------------------
# Build the datasets for the PART II opener.
#
...
# THE ARGUMENT.
#
#   NOBODY PRODUCES THIS DATA. That is the whole part. There is no national
#   election administrator in the United States, so no agency publishes national
#   returns. Every county-level file anyone has ever used was assembled
#   privately out of fifty-one separate state records. Part I's data is
#   compelled and CENTRAL: one bureau, one file. Part II's is compelled and
#   SCATTERED: certified everywhere, collected nowhere.
#
# ---------------------------------------------------------------------------
# SOURCES
# ---------------------------------------------------------------------------
...
# THE BEAT MAP COVERS THE FIRST RUN ONLY, and that is the shape of a part with
# two instrument families in it. Elections arrive twice: as a RESULT that was
# certified, and as the MACHINERY that produced it. The result run opens the
# part and takes the beats. The machinery run opens later, at
# voter-files-source, and its own source/access/instrument chapters stay OUT of
# this map deliberately -- labelling them would drop a "1 source" into the
# middle of the applied chapters and fail the contiguity test below for an
# order that is deliberately right.
BEAT <- c(
  "returns-source"    = "1 source",
  "county-returns"    = "2 instrument",
  "data-sources"      = "3 access")
#
# THIS PART HAS A SECOND SOURCE CHAPTER, and it is not in the beat map.
# rosters-source opens the run on midterm-loss, retirements, primary-defeats
# and careers -- four chapters that do not use returns at all. They run on a
# ROSTER of who held the seat, which is a different question from who won the
# election, and the gap between the two is 2,344 people. It is a source chapter
# by kind and a mid-part run opener by position, so like eavs in Part IV it
# stays out of the beat: the beat vocabulary describes one run per part, and
# this part runs several.
#
# AND A THIRD, clerk-source, which opens the run on house-competition. The
# Clerk of the House publishes the one federal compilation of congressional
# returns, and publishes it as a typeset DOCUMENT rather than a table; that
# chapter is about what it costs to read one. house-competition used to sit
# inside the roster run, where it did not belong -- it reads the Clerk, not a
# roster -- and now sits with its own source, taking its companion
# `distributions` with it.
#
# So Part II runs FOUR sources: returns (the part opener's), the Clerk, the
# rosters, and then the precinct and ballot chapters at the end. Only the first
# is in the beat map, because the beat vocabulary describes one run per part.
...
COMPANION <- c("sparklines", "distributions")
