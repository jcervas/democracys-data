# ---------------------------------------------------------------------------
# Geometry for the figures in precinct-geography-brief.Rmd
#
# The brief is a knitted document that must build to HTML *and* PDF and must not
# carry a shapefile inside it. This script does all the spatial work once and
# writes small, plain CSVs of coordinates that base R can `polygon()` and D3 can
# turn into paths. Nothing downstream needs sf.
#
#   derived/fig_forsyth.csv        Forsyth County precinct outlines, 2020 and 2024
#   derived/fig_forsyth_lab.csv    a label point per precinct, flagged new / retired
#   derived/fig_wind_blocks.csv    the 96 census blocks inside FORSYTH|25 WINDERMERE
#   derived/fig_wind_pts.csv       one interior point per block, with population
#   derived/fig_wind_meta.csv      the numbers quoted in the caption
#
# WHY FORSYTH. It is the clearest instance of the brief's claim that precincts
# split where population grows: 20 precincts in 2020, 29 in 2024, in a county of
# 251,283 people on Atlanta's northern edge. Nothing about the county line moved.
#
# WHY WINDERMERE. It splits two ways by population and is the sharpest case of
# area and population disagreeing: land says 54/44, people say 75/25.
#
# SIMPLIFICATION. Both years are simplified in ONE mapshaper call so that shared
# arcs are detected across the two layers. Boundaries that genuinely did not move
# are therefore simplified identically in both panels, and the only differences
# visible between the panels are real ones. Simplifying the years separately
# would manufacture wobble on unchanged lines -- exactly the thing the figure is
...
CTY   <- "FORSYTH"
FOCUS <- "FORSYTH|25 WINDERMERE"
UTM   <- 26916                      # UTM 16N, metres, correct for north Georgia
...
p20 <- rd("raw/shp2020", "\\.shp$")
p24 <- rd("raw/shp2024", "\\.shp$")
p20$key <- toupper(trimws(paste(p20$CTYNAME, p20$PRECINCT_N, sep = "|")))
p24$key <- toupper(trimws(paste(p24$CTYNAME, p24$PRECINCT_N, sep = "|")))

f20 <- p20[!is.na(p20$CTYNAME) & toupper(p20$CTYNAME) == CTY, c("key")]
f24 <- p24[!is.na(p24$CTYNAME) & toupper(p24$CTYNAME) == CTY, c("key")]
...
sm   <- st_cast(ms_simplify(both, keep = 0.04, keep_shapes = TRUE, explode = FALSE),
                "MULTIPOLYGON")
cat(sprintf("simplified: %s -> %s coordinates\n",
            format(nrow(st_coordinates(both)), big.mark = ","),
            format(nrow(st_coordinates(sm)),   big.mark = ",")))

# origin so the written numbers are small; km, 1 m resolution
...
dd_write_csv(out[, c("yr", "name", "part", "x", "y")], "derived/fig_forsyth.csv")
cat(sprintf("fig_forsyth.csv  %s rows, %s\n", format(nrow(out), big.mark = ","),
            format(structure(file.size("derived/fig_forsyth.csv"), class = "object_size"), units = "auto")))

# ---- 2. label points, and which names are new / retired -------------------
lab <- function(g, yr, key) {
  p <- suppressWarnings(st_coordinates(st_point_on_surface(g)))
  data.frame(yr = yr, name = sub("^[^|]*\\|", "", key),
             x = round((p[1, "X"] - OX) / 1000, 3), y = round((p[1, "Y"] - OY) / 1000, 3))
}
L <- do.call(rbind, lapply(seq_len(nrow(sm)),
       function(i) lab(st_geometry(sm)[i], sm$yr[i], sm$key[i])))
...
POP <- read.csv("derived/crosswalk_pop.csv", stringsAsFactors = FALSE)
POP <- POP[POP$weight >= 0.01, ]
ntg <- table(POP$from_2020)
L$split <- ifelse(L$yr == 2020,
                  ntg[paste0(CTY, "|", L$name)] > 1, NA)
L$split[is.na(L$split)] <- FALSE
dd_write_csv(L, "derived/fig_forsyth_lab.csv")
...
ba <- read.csv("derived/block_assign.csv", stringsAsFactors = FALSE, colClasses = c(GEOID20 = "character"))
keep <- ba[!is.na(ba$precinct_2020) & ba$precinct_2020 == FOCUS, ]
cat(sprintf("\n%s: %d blocks, %s people\n", FOCUS, nrow(keep), format(sum(keep$pop), big.mark = ",")))
...
# short target label: the 2024 precinct each block landed in
...
dd_write_csv(B, "derived/fig_wind_blocks.csv")
...
dd_write_csv(P, "derived/fig_wind_pts.csv")
...
dd_write_csv(DS[, c("tgt", "id", "part", "x", "y")], "derived/fig_wind_outline.csv")
cat(sprintf("fig_wind_blocks.csv %s rows, %s\n", format(nrow(B), big.mark = ","),
            format(structure(file.size("derived/fig_wind_blocks.csv"), class="object_size"), units="auto")))
...
po <- read.csv("derived/crosswalk_pop.csv", stringsAsFactors = FALSE)
...
M$w_pop[is.na(M$w_pop)] <- 0; M$pop[is.na(M$pop)] <- 0
...
dd_write_csv(M, "derived/fig_wind_meta.csv")
