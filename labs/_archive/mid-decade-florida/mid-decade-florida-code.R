# mid-decade-florida-code.R -- chunk bodies for mid-decade-florida-brief.Rmd
#
...
OFF_ORDER <- c("president", "senator", "gov", "ag", "trea")
el_office <- sub("-[0-9]{4}$", "", seats$election)
seats <- seats[order(seats$year, match(el_office, OFF_ORDER)), ]
row.names(seats) <- NULL

# Florida elects a Chief Financial Officer, a real office this collection's
# "trea" shorthand can be given its actual name for -- unlike the Texas
# chapter, which has no matching office and keeps the shorthand verbatim.
OFFICE <- c(president = "President", senator = "U.S. Senate", gov = "Governor",
            ag = "Attorney General", trea = "Chief Financial Officer")
el_label <- function(e)
...
geo <- jsonlite::fromJSON("data/derived/fl-districts-tween.json",
...
# every (old, new) district pair with a shared population, from the census-
# block assignment behind the crosswalk -- not just the winning pair
overlap <- read.csv("data/derived/fl_overlap_pop.csv", stringsAsFactors = FALSE)
predecessor_of <- setNames(
  vapply(geo$districts, function(d) if (!is.null(d$old_id)) d$old_id else d$id, 1),
  vapply(geo$districts, function(d) d$id, 1))

# race/ethnicity composition per district, both plans -- straight off the
# Dave's Redistricting exports, keyed to the same 1-28 district ids
demo <- read.csv("data/derived/fl_demographics.csv", stringsAsFactors = FALSE)
DO <- demo[demo$plan == "old", ]; DO <- DO[order(DO$district), ]
DN <- demo[demo$plan == "new", ]; DN <- DN[order(DN$district), ]
DEMO_CATS <- c(white = "White", black = "Black", hispanic = "Hispanic",
                asian = "Asian", native = "Native American", pacific = "Pacific Islander")
...
for (plan in c("old", "new")) {
  ...
  mtext(if (plan == "old") "2022 lines" else "2026 lines",
...
## ---- mm-panel-static
# print fallback for the district panel: district 25, the online default
d25 <- long[long$district == 25, ]
d25 <- d25[match(seats$election, d25$election), ]
par(mar = c(4, 10, 2.2, 2))
yy <- rev(seq_len(nrow(d25)))
plot(NA, xlim = c(30, 65), ylim = range(yy) + c(-0.5, 0.5),
     axes = FALSE, ann = FALSE)
abline(v = 50, col = "#777777", lty = 3)
segments(100 * d25$share_old, yy, 100 * d25$share_new, yy,
         col = "#999999", lwd = 1.6)
points(100 * d25$share_old, yy, pch = 21, bg = "#FFFFFF", col = "#4E5A63", cex = 1.1)
points(100 * d25$share_new, yy, pch = 19,
       col = ifelse(d25$share_new > 0.5, "#2C7FB8", "#C41230"), cex = 1.1)
axis(1, at = seq(30, 65, 5), labels = paste0(seq(30, 65, 5), "%"), cex.axis = 0.8)
mtext("Democratic share of the two-party vote, district 25", side = 1,
      line = 2.4, cex = 0.85)
text(29, yy, seats$label, xpd = NA, adj = 1, cex = 0.72)
legend("bottomleft", c("2022 predecessor", "2026 lines"), pch = c(21, 19),
       col = c("#4E5A63", "#C41230"), pt.bg = c("#FFFFFF", NA),
       bty = "n", cex = 0.78)

## ---- mm-d3
GEO <- paste(readLines("data/derived/fl-districts-tween.json", warn = FALSE),
             collapse = "")
sh_row <- function(m, i) paste(sprintf("%.4f", as.numeric(m[i, ])), collapse = ",")
SH <- paste(vapply(seq_len(28), function(i)
...
sel.property("value","president-2024");
...
const bOld=bar.append("button").attr("style",bstyle).text("2022 lines");
const bNew=bar.append("button").attr("style",bstyle).text("2026 lines");
...
dsel.selectAll("option").data(d3.range(1,29)).join("option").attr("value",d=>d).text(d=>"District "+d);
dsel.property("value","25");
...
window.__FLGEO=G;
let elx="president-2024",plan=1,selD=25;   // plan: 0 = 2022 lines, 1 = 2026 lines
...
const oldIdOf=(id)=>{const d=G.districts.find(x=>x.id===id);return (d&&d.old_id)?d.old_id:id;};
...
paths.on("mouseenter",function(e,d){
    ...
    const oid=oldIdOf(d.id);
    const from=(oid===d.id)?("District "+d.id):("District "+d.id+" (mostly old district "+oid+")");
    cap.html("<b>"+from+"</b>, "+EL[EIX[elx]].t+": "+
      (100*o).toFixed(1)+"% Democratic under its 2022 predecessor, "+
      (100*n).toFixed(1)+"% under the 2026 lines"+
...
function panel(){
  ...
  psvg.append("text").attr("x",pm.l).attr("y",36).attr("font-size","11px").attr("fill","#777")
    .text((oid===selD?"open circle: 2022 lines":"open circle: old district "+oid)+
      " \\u00b7 filled: 2026 lines \\u00b7 right of the line = Democratic candidate ahead");
  ...
  ins.append("text").attr("x",ix).attr("y",iy+IS+26).attr("font-size","11px").attr("fill","#777")
    .text(oid===selD?"dashed: 2022 lines \\u00b7 solid: 2026 lines":
      "dashed: old district "+oid+" \\u00b7 solid: 2026 lines");
  ins.append("text").attr("x",ix).attr("y",iy+IS+42).attr("font-size","11px").attr("fill","#777")
    .text("fill: the selected race under the 2026 lines");
}
...
plot(NA, xlim = c(15, 25), ylim = range(yy) + c(-0.5, 0.5),
     axes = FALSE, ann = FALSE)
abline(v = 15:25, col = "#EEEEEE")
...
axis(1, at = seq(15, 25, 2), cex.axis = 0.8)
mtext("Republican seats out of 28", side = 1, line = 2.4, cex = 0.85)
text(14.6, yy, seats$label, xpd = NA, adj = 1, cex = 0.72)
text(seats$rep_new + 0.35, yy, paste0("+", seats$rep_gain), adj = 0,
     cex = 0.72, col = "#555555", xpd = NA)
legend("topleft", c("2022 lines", "2026 lines"), pch = c(21, 19),
       col = c("#4E5A63", "#C41230"), pt.bg = c("#FFFFFF", NA),
       bty = "n", cex = 0.78)
...
(function(){
const R=[', SB, '];
const W=760,H=390,M={t:30,r:70,b:40,l:150};
...
const DEF="Open circle: Republican seats under the 2022 lines. Filled red: the same "+
  "votes under the 2026 lines. <i>Hover a row for the counts.</i>";
cap.html(DEF);
const x=d3.scaleLinear().domain([15,25]).range([M.l,W-M.r]);
...
svg.append("text").attr("x",(M.l+W-M.r)/2).attr("y",H-6).attr("text-anchor","middle")
  .attr("font-size","12px").attr("fill","#555").text("Republican seats out of 28");
const rg=svg.selectAll("g.r").data(R).join("g")
  .on("mouseenter",function(e,r){
    d3.select(this).selectAll("circle").attr("r",6.5);
    cap.html("<b>"+r.t+"</b>: "+r.o+" Republican seats under the 2022 lines, "+
      r.n+" under the 2026 lines \\u2014 a gain of "+r.g+".");})
  .on("mouseleave",function(){
...
plot(so, sn, xlim = c(15, 85), ylim = c(15, 85), pch = 19, cex = 0.55,
     col = adjustcolor(cl, 0.7), axes = FALSE,
     xlab = "Democratic share, 2022 lines (%)",
     ylab = "Democratic share, 2026 lines (%)")
...
const sel=bar.append("select").attr("style","font:inherit;padding:2px 4px");
sel.selectAll("option").data(["All districts",...d3.range(1,29).map(d=>"District "+d)])
  .join("option").attr("value",(d,i)=>i).text(d=>d);
const svg=box.append("svg").attr("viewBox",`0 0 ${W} ${H}`)
  .attr("style","max-width:560px;width:100%;height:auto;font:12px inherit");
const cap=box.append("p").attr("style","font-size:0.86em;color:#444;min-height:2.2em;margin:0.3em 0 0 0");
const DEF="One point per district per election. Below the diagonal, the 2026 lines "+
...
svg.append("text").attr("x",(M.l+W-M.r)/2).attr("y",H-8).attr("text-anchor","middle")
  .attr("font-size","12px").attr("fill","#555").text("Democratic share, 2022 lines");
svg.append("text").attr("transform",`translate(14,${(M.t+H-M.b)/2}) rotate(-90)`)
  ...
  .text("Democratic share, 2026 lines");
...
sel.on("change",function(){hi=+this.value;restyle();
  cap.html(hi?("<b>District "+hi+"</b> highlighted: its thirteen elections among all "+P.length+" points."):DEF);
...
## ---- ov-prep
# FOCUS + CONTEXT. An even alluvial across 28 districts gives each one
# 1/28th of the height, which buries the only thing worth seeing: the
...
ov_layout <- function(fl, spot, focus = 0.62, gapf = 0.006) {
...
for (i in 1:NB) {
  ...
  text(NWD/2, OV$topL[i] + OV$hL[i]/2, i,
       cex = if (OV$hL[i] > 0.02) 0.62 else 0.5,
  ...
  text(1 - NWD/2, OV$topR[i] + OV$hR[i]/2, i,
       cex = if (hl) 0.72 else if (OV$hR[i] > 0.02) 0.62 else 0.5,
       font = if (hl) 2 else 1,
...
mtext("2022 districts", side = 3, at = 0, line = 0.1, cex = 0.72, adj = 0)
mtext("2026 districts", side = 3, at = 1, line = 0.1, cex = 0.72, adj = 1)

## ---- ov-d3
FLOWS <- paste(sprintf('{"o":%d,"n":%d,"w":%.5f}',
                       flows$old, flows$new, flows$weight), collapse = ",")
PRED <- paste(sprintf('"%d":%d', as.integer(names(predecessor_of)), predecessor_of),
              collapse = ",")
cat(paste0('
...
const F=[', FLOWS, '];
const PRED={', PRED, '};
const NB=28,FOCUS=0.62,GAPF=0.006;
const W=760,NWD=44,TOPM=22,HGT=740;
...
const sel=bar.append("select").attr("style","font:inherit;padding:2px 4px");
sel.selectAll("option").data(d3.range(1,29)).join("option").attr("value",d=>d).text(d=>"District "+d);
sel.property("value","25");
const svg=box.append("svg").attr("viewBox",`0 0 ${W} ${HGT+TOPM+14}`)
  .attr("style","max-width:100%;height:auto;font:12px inherit");
const cap=box.append("p").attr("style","font-size:0.86em;color:#444;min-height:3.3em;margin:0.3em 0 0 0");
const DEF="Every ribbon is a group of people: it leaves the 2022 district that held "+
  "them and lands in the 2026 district that holds them now. The selected district is "+
...
cap.html(DEF);
// ---- focus+context layout: ribbons into `spot` are scaled up, the rest collapse
let hL=[],hR=[],topL=[],topR=[];
...
svg.append("text").attr("x",0).attr("y",14).attr("font-size","11.5px")
  .attr("font-weight","700").attr("fill","#4E5A63").text("2022 districts");
...
const rib=svg.append("g").selectAll("path").data(F).join("path")
  ...
  .on("mouseenter",function(e,f){
    ...
    cap.html("<b>"+(100*f.w).toFixed(1)+"%</b> of 2022 district "+f.o+
      "\\u2019s people are in 2026 district "+f.n+
      (PRED[f.n]===f.o?" \\u2014 the predecessor this chapter uses for district "+f.n+".":".")+
...
mkNode(gL,0,false); mkNode(gR,W-NWD,true);
function shade(){
...
function place(t){
  ...
  R(gL.selectAll("text")).attr("y",d=>(topL[d]+hL[d]/2)*HGT+TOPM+4)
    .attr("font-size",d=>hL[d]*HGT>16?"11px":"8px")
  ...
  R(gR.selectAll("text")).attr("y",d=>(topR[d]+hR[d]/2)*HGT+TOPM+4)
    .attr("font-size",d=>d===selD?"13px":hR[d]*HGT>16?"11px":"8px")
...
## ---- demo-static
delta <- 100 * (DN$minority_pct - DO$minority_pct)
ord <- order(delta)
par(mar = c(4, 4.5, 1, 1))
cols <- ifelse(delta[ord] < 0, "#B2182B", "#2166AC")
barplot(delta[ord], horiz = TRUE, names.arg = paste("District", (1:28)[ord]),
        cex.names = 0.6, las = 1, col = cols, border = NA,
        xlab = "Change in non-white population share, 2022 lines to 2026 lines (points)")
abline(v = 0, col = "#777777")

## ---- demo-d3
CATS <- names(DEMO_CATS)
demo_row <- function(did) {
  o <- sapply(CATS, function(c) DO[[paste0(c, "_pct")]][did])
  n <- sapply(CATS, function(c) DN[[paste0(c, "_pct")]][did])
  paste(sprintf('{"k":"%s","o":%.4f,"n":%.4f}', DEMO_CATS, o, n), collapse = ",")
}
DDEMO <- paste(sprintf('{"d":%d,"rows":[%s]}', 1:28, vapply(1:28, demo_row, character(1))),
               collapse = ",")
cat(paste0('
<div id="demo" style="position:relative;margin:1em 0"></div>
<script>
(function(){
const DD=[', DDEMO, '];
const W=680,M={t:14,r:70,b:44,l:130};
const box=d3.select("#demo");
window.DD_SYNC=window.DD_SYNC||{fns:[],select(id){this.fns.forEach(fn=>fn(id));},
  subscribe(fn){this.fns.push(fn);}};
const bar=box.append("div").attr("style","margin-bottom:6px;font-size:13px;display:flex;gap:8px;align-items:center");
bar.append("span").text("District:");
...
sel.selectAll("option").data(d3.range(1,29)).join("option").attr("value",d=>d).text(d=>"District "+d);
sel.property("value","25");
const svg=box.append("svg").attr("style","max-width:100%;height:auto;font:12px inherit");
const cap=box.append("p").attr("style","font-size:0.86em;color:#444;min-height:2.2em;margin:0.3em 0 0 0");
const x=d3.scaleLinear().domain([0,100]).range([M.l,W-M.r]);
function draw(did){
  const R=DD[did-1].rows;
  const DEF="Open circle: 2022 lines. Filled: 2026 lines. <i>Hover a row for the shares.</i>";
  cap.html(DEF);
  const rh=34,H=M.t+R.length*rh+M.b;
  const y=d3.scaleBand().domain(R.map(r=>r.k)).range([M.t,M.t+R.length*rh]).padding(0.35);
  svg.attr("viewBox",`0 0 ${W} ${H}`);
  svg.selectAll("*").remove();
  svg.append("g").attr("transform",`translate(0,${M.t+R.length*rh})`)
    .call(d3.axisBottom(x).ticks(5).tickFormat(v=>v+"%"));
  svg.append("text").attr("x",(M.l+W-M.r)/2).attr("y",H-6).attr("text-anchor","middle")
    .attr("font-size","12px").attr("fill","#555").text("Share of district population, district "+did);
  const rg=svg.selectAll("g.r").data(R).join("g")
    .on("mouseenter",function(e,r){
      d3.select(this).selectAll("circle").attr("r",6.5);
      cap.html("<b>"+r.k+"</b>, district "+did+": "+(100*r.o).toFixed(1)+
        "% under the 2022 lines, "+(100*r.n).toFixed(1)+"% under the 2026 lines.");})
    .on("mouseleave",function(){
      d3.select(this).selectAll("circle").attr("r",5);cap.html(DEF);});
  rg.append("rect").attr("x",M.l-120).attr("width",W-M.r-M.l+190)
    .attr("y",r=>y(r.k)).attr("height",y.bandwidth()+6).attr("fill","transparent");
  rg.append("text").attr("x",M.l-10).attr("y",r=>y(r.k)+y.bandwidth()/2+4)
    .attr("text-anchor","end").attr("font-size","11.5px").attr("fill","#4E5A63").text(r=>r.k);
  rg.append("line").attr("x1",r=>x(100*r.o)).attr("x2",r=>x(100*r.n))
    .attr("y1",r=>y(r.k)+y.bandwidth()/2).attr("y2",r=>y(r.k)+y.bandwidth()/2)
    .attr("stroke","#999").attr("stroke-width",1.6);
  rg.append("circle").attr("cx",r=>x(100*r.o)).attr("cy",r=>y(r.k)+y.bandwidth()/2)
    .attr("r",5).attr("fill","var(--paper,#fff)").attr("stroke","#4E5A63").attr("stroke-width",1.6);
  rg.append("circle").attr("cx",r=>x(100*r.n)).attr("cy",r=>y(r.k)+y.bandwidth()/2)
    .attr("r",5).attr("fill","#4E5A63");
}
sel.on("change",function(){draw(+this.value);window.DD_SYNC.select(+this.value);});
window.DD_SYNC.subscribe(id=>{sel.property("value",id);draw(id);});
draw(25);
})();
</script>'))
