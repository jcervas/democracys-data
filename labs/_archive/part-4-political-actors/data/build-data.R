# ---------------------------------------------------------------------------
# Build the datasets for the PART IV opener.
#
...
# THE ARGUMENT.
#
#   TWO PROVENANCES, DELIBERATELY IN ONE PART. Half of this data exists because
#   somebody was legally required to DISCLOSE it -- campaign finance, lobbying.
#   The other half exists because somebody chose to publish something they
#   were never obliged to file at all -- pageviews, visit trackers, market
#   prices. The contrast is the argument: disclosure is not audit, and
#   commercial data is not disinterested.
#
# ---------------------------------------------------------------------------
# SOURCES
# ---------------------------------------------------------------------------
...
# SAME TWO-RUN SHAPE AS PART III. The voting record opens the part and takes
# the beats; the money run opens later at finance-source and stays out of the
# map for the same reason. What holds the two together is not the instrument
# but WHO THE ROW IS ABOUT: a member of Congress, a committee, a candidate --
# somebody who chose to run for office. That is the line between this part and
# the next one, where nobody volunteered for anything.
BEAT <- c(
  "rollcalls-source" = "1 source",
  "dw-nominate"      = "2 instrument",
  "scdb"             = "2 instrument")
...
COMPANION <- c("pie-radar", "streamgraph", "rank-size")
