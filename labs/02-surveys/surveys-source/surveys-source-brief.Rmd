...
A **margin of error** says how far off a sample's estimate could be, and still be doing its job. `r nn(NRESP)` respondents give a national estimate accurate to about **± `r p1(ALLMOE)` points**, which is excellent. Split those same respondents by state and the median state — the one in the middle, with as many states above it as below — holds **`r nn(MEDN)` people**, giving **± `r p1(MEDMOE)` points**. That range is so wide that a result of 50% is consistent with a true value anywhere from a third to two-thirds. The smallest state has `r nn(MINN)` respondents and a margin of ± `r p1(MINMOE)` points, which is not a measurement at all.

The arithmetic behind those margins is short enough to do by hand. For a result near 50%, the margin is 1.96 times the square root of 0.25 divided by the number of respondents. Take the median state's `r nn(MEDN)` people: 0.25 divided by `r nn(MEDN)` is `r formatC(MED_VAR, format = "f", digits = 4)`, its square root is `r formatC(MED_SE, format = "f", digits = 3)`, and 1.96 times that is `r formatC(1.96 * MED_SE, format = "f", digits = 3)` — which, as a percentage, is the ± `r p1(MEDMOE)` points in the table. Nothing about the state is being measured badly. There are simply too few people in the cell for the answer to be narrow.
...
The table is drawn from the 2024 Cooperative Election Study, which publishes each answer's share twice: once as a plain count of the people interviewed, and once after weighting. In it, **`r p1(WREG$pct_unweighted)`% of the people interviewed said they were registered to vote, and the published estimate is `r p1(WREG$pct_weighted)`%** — an adjustment of `r p1(abs(WREG$adjustment))` points on a single item. That is not evidence of a problem: it is the correction working, and a survey that reported the raw figure would be badly wrong about the country. But it means the weights do a great deal of work before any result appears, and a reader who does not know they exist is reading a number that is substantially a model.

Follow one respondent through it. She was asked, agreed, and told the CES she is not registered to vote. The `r nn(WNO$n)` people who said that are `r p1(WNO$pct_unweighted)`% of everyone interviewed. Nobody set a target for registration; the weights aim at the census's counts of adults by age, sex, race and education. But the people the raw sample was short of — the young, the less educated, the less attentive to politics — are also the people least likely to be registered, so when their answers are counted up, so is non-registration, and the weighted file says `r p1(WNO$pct_weighted)`% of adults are unregistered. On average, then, each unregistered respondent's answer is counted as about `r p1(W_NO)` answers, and each registered respondent's as about `r p2(W_REG)` of one. She was asked as one person. In the published estimate she stands in for three, because the sample caught only about a third as many people like her as the country holds. The [weighting chapter](../ces-class/ces-class-brief.html) opens that machinery fully.
...
**A survey records what people say, not what they did.** Whether a stated answer matches a stable underlying attitude, or is partly invented on the spot, is a real and unsettled argument,^[The case that answers are assembled on the spot from whatever considerations happen to be in mind is John Zaller and Stanley Feldman, "A Simple Theory of the Survey Response: Answering Questions versus Revealing Preferences," *American Journal of Political Science* 36, no. 3 (1992): 579–616, <https://doi.org/10.2307/2111583>.] taken up in the [ANES chapter](../anes/anes-brief.html). Whether it matches behavior is checkable only by joining a survey to a record of the same people, and that audit waits for Section IV.
...
- Pew Research Center (2017). "What Low Response Rates Mean for Telephone
  Surveys." Who still answers, measured against census benchmarks.
