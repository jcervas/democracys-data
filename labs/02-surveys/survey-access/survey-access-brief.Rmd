...
The measurement is a scan of six addresses, made on `r FV("scan_date")` and kept frozen at that date. The addresses are the ones this book's own chapters read: the Census Bureau's CPS voting supplement table, the GSS cumulative file, and the ANES cumulative file's download page. To them add the Cooperative Election Study on Harvard Dataverse, a second Dataverse dataset, and an ICPSR study page. These are the archives behind [everything a survey can establish that no record can](../surveys-source/surveys-source-brief.html), which is why their front doors are worth a chapter of their own. If the section's data lives behind these six addresses, the six addresses are the section's infrastructure.
