...
And sometimes no crosswalk can exist at the level you want. Connecticut's planning regions are not the old counties regrouped: the boundaries cut across each other, so no county-to-region lookup table is possible, with or without weights. The honest translation has to descend to census blocks, a smaller unit that nests inside both, and rebuild each side from below. That is the general lesson. A crosswalk between two sets of areas is really a claim that some smaller unit tiles both, and when none does, the translation is a modelling exercise wearing a table's clothing.
