...
Except one. The red ribbon is a single row whose code survives intact while the place underneath it changes — the District of Columbia collision that [this book's introduction](../../00-front-matter/introduction/introduction-brief.html) opens on, where `11001` means the whole city in one file and one ward of it in the other. It is the only failure in this figure that a row count cannot see, which is exactly why the introduction gave it the first pages of the book. A join key is only as good as the promise that the same string means the same thing on both sides, and **that promise is not stored anywhere.** No column records it, and no software can check it.
