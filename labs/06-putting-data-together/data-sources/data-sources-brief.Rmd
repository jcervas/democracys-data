...
The Bureau publishes the county list every year as Gazetteer files: one row per county-equivalent, tab-separated, the code in a column called `GEOID`. This chapter reads the 2024 edition. Beside it sit two files that carry the same code from a different world: Tony McGovern's county-level compilation of the 2020 and 2024 presidential results. He scraped them from news organisations' election-night pages, and the README states plainly that the numbers are not authoritative. The pairing is the point. An official list of what the code means, and a working file that uses it, is the situation every join in this section is in.
